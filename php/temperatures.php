<?php
    ...
    define("NOC", 3);                                                           // number of cells to build average on
    ...
    function get_data( &$string, &$array, &$min, &$max, &$x_of_max, $trans )
        ...
        while( (!feof($file)) && ($j < MAX_ENTRIES+2) )
            ...
            if( $l != 0 )
                ...
                if( $j != 0 )
                    {
                    if( ( (float)$a[$k] < ((float)$t_old - 7.0) ) || ( (float)$a[$k] > ((float)$t_old + 7.0) ) )
                        continue;
                    else
                        $t_old = $a[$k];
                    }
    ...
    function show( &$array, $min, $diff, $illegal, $image, $color )
        ...
        for( $j=0; $j<XDIFF-1; $j++ )
            ...
            if( ((float)$array[$j] != $illegal) && ((float)$array[$j+1] != $illegal) )
    ...
    ImageString($im, FONT, $x, YABS+YDIFF+6+ImageFontHeight(FONT)+3, $string, $blue);
    ...
    $x = XABS - 8 * ImageFontWidth(FONT);
    ImageString($im, FONT, $x, YABS+YDIFF+6+2*(ImageFontHeight(FONT)+3), "(C) www.ur9.de", $black);
